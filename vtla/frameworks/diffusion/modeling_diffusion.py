...
# Per-tactile-key projection width added to the diffusion global conditioning vector.
DIFFUSION_TACTILE_PROJ_DIM = 128
...
class DiffusionPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        if self.config.image_features:
            ...
            batch = _resize_images_to_common(batch, self.config.image_features, self.config.resize_imgs_to)
...
def _resize_images_to_common(
    batch: dict[str, Tensor], image_keys, size: tuple[int, int] | None
) -> dict[str, Tensor]:
    """Bilinearly resize every camera tensor in `batch` to `size` (H, W) so that cameras
    with different native resolutions can be stacked into a single tensor.

    Handles arbitrary leading dims, i.e. (B, C, H, W) at inference and
    (B, n_obs_steps, C, H, W) during training.
    """
    if size is None:
        return batch
    ...
    for key in image_keys:
        ...
        batch[key] = flat.reshape(*lead, c, target_h, target_w)
    return batch
...
class DiffusionModel(nn.Module):
    def __init__(self, config: DiffusionConfig):
        ...
        # `robot_state_feature` is None when state_mode="none".
        global_cond_dim = 0
        if self.config.robot_state_feature:
    ...
    def _obs_batch_dims(self, batch: dict[str, Tensor]) -> tuple[int, int]:
        """(batch_size, n_obs_steps) read from whichever observation is present."""
        ref = OBS_STATE if self.config.robot_state_feature else (
            OBS_IMAGES if self.config.image_features else OBS_ENV_STATE
        )
        return batch[ref].shape[:2]

    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        """Encode image features and concatenate them all together along with the state vector."""
        batch_size, n_obs_steps = self._obs_batch_dims(batch)
        global_cond_feats = [batch[OBS_STATE]] if self.config.robot_state_feature else []
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        _, n_obs_steps = self._obs_batch_dims(batch)
